//! wasm-bindgen-test coverage for the workflow-wasm `Cast` family
...
//!
//! Run with:  `wasm-pack test --node tests/wasm32`
...
/// A `CastTarget` created in Rust and handed to JS as a class instance casts back
/// to the same Rust object *by reference* — this is the `WasmPtr<WasmRefCell<T>>`
/// reference ABI path.
#[wasm_bindgen_test]
fn cast_from_instance_resolves_by_reference() {
    ...
    assert!(matches!(cast, Cast::Ref { .. }), "expected a borrowed reference");
    assert_eq!(cast.into_owned().value(),42);
}

/// A non-instance JsValue (a number) falls through to the user `create` closure
/// and is constructed into an owned value.
#[wasm_bindgen_test]
fn cast_from_number_constructs_value() {
    ...
    assert!(matches!(cast, Cast::Value { .. }), "expected an owned value");
    assert_eq!(cast.into_owned().value(),7);
}

/// `try_owned_from` consumes the cast and yields an owned `CastTarget` for both a
/// real instance and the constructed-from-number fallback.
#[wasm_bindgen_test]
fn try_owned_from_yields_owned_value() {
    ...
    assert_eq!(
        CastTarget::try_owned_from(&JsValue::from_f64(5.0)).unwrap().value(),
        5
    );
}
