...
extern "C" {
    // Renamed from `WriteStream` to avoid a JS class-name collision with
    // `fs::WriteStream` (which owns a `#[wasm_bindgen] impl` block). wasm-bindgen
