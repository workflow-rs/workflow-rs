...
pub fn try_get_bool_from_prop(jsv: &JsValue, prop: &str) -> Result<bool, JsValue> {
    Reflect::get(jsv, &JsValue::from(prop))?
        .as_bool()
        .ok_or_else(|| {
            JsValue::from(format!(
...
pub fn try_get_vec_u8_from_prop(jsv: &JsValue, prop: &str) -> Result<Vec<u8>, JsValue> {
...
pub fn try_get_vec_u8_from_bn_prop(object_jsv: &JsValue, prop: &str) -> Result<Vec<u8>, JsValue> {
...
pub fn try_get_vec_u8_from_bn(bn_jsv: &JsValue) -> Result<Vec<u8>, JsValue> {
    let bytes = apply_with_args0(bn_jsv, "toBytes")?;
    let array = Uint8Array::new(&bytes);
    Ok(array.to_vec())
}
...
pub fn try_get_string_from_prop(jsv: &JsValue, prop: &str) -> Result<String, JsValue> {
    let str = Reflect::get(jsv, &JsValue::from(prop))?;
    match str.as_string() {
        Some(str) => Ok(str),
